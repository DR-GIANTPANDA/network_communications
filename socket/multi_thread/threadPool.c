...
typedef struct Task
{
    void (*function)(void* arg); // 给任务队列里添加的是任务不同的函数
    void* arg;  // 这是任务函数的参数
}Task;
...
struct ThreadPool
{
    ...
    pthread_mutex_t mutexPool;  // 锁整个线程池的互斥锁
    pthread_mutex_t mutexBusy;  // 锁正在忙的线程 因为任务处理的过程中这个变量一直在变化

    // 条件变量有上下限，所有需要2个
    pthread_cond_t notFull;   // 条件变量判断任务是否满了 如果不满则可以继续添加工作任务
    pthread_cond_t notEmpty;  // 条件变量判断任务是否为空，如果空则不可以使用工作线程
